...
must_haves:
  truths:
    - "BUG-01 er det første punkt i ISSUES.md under en 'Kritisk / Tidskritisk'-overskrift"
    - "BUG-01-indgangen viser datoen 29. marts 2026 eksplicit som deadline"
    - "Ole kan se alle fem kendte fejl (BUG-01 til BUG-05) med symptom, berørte filer og løsningsvej"
    - "Teknisk gæld er organiseret i en separat sektion efter fejlene"
    - "Ole kan bruge listen til at prioritere hvad der skal fixes"
  artifacts:
    - path: "docs/ISSUES.md"
      provides: "Prioriteret liste over kendte fejl og teknisk gæld"
      min_lines: 80
  key_links:
    - from: "docs/ISSUES.md"
      to: ".planning/codebase/CONCERNS.md"
      via: "Alle bugs og tech debt hentet fra CONCERNS.md Known Bugs og Tech Debt sektioner"
      pattern: "BUG-0[1-5]"
    - from: "docs/ISSUES.md"
      to: ".planning/ROADMAP.md"
      via: "Fase-referencer for løsninger matcher ROADMAP.md faseinddeling"
      pattern: "Fase 2|Fase 3|Fase 4"
---

<objective>
Skriv docs/ISSUES.md — en prioriteret dansk liste over kendte fejl og teknisk gæld i O2A, beregnet til Ole som beslutningsgrundlag for hvad der skal rettes.

Purpose: Ole har brug for et enkelt dokument der giver ham overblik over hvad der er galt, hvor alvorligt det er, og hvad der er planlagt rettet hvornår. BUG-01 er tidskritisk — sommertid 2026 starter 29. marts 2026 (13 dage fra kortlægningstidspunktet) og skal fremgå tydeligt.

Output: docs/ISSUES.md (ny fil)
</objective>
...
<task type="auto">
  <name>Task 1: Skriv ISSUES.md med kendte fejl og teknisk gæld</name>
  <files>docs/ISSUES.md</files>
  <action>
Opret docs/ISSUES.md (docs/-mappen oprettes af plan 01-01 — hvis den ikke eksisterer endnu, opret den). Skriv filen på dansk med følgende struktur — BUG-01 SKAL være første indgang under KRITISK-overskriften:

**# Kendte problemer**

Kort intro: "Oversigt over kendte fejl og teknisk gæld i O2A pr. 2026-03-16. Sorteret efter prioritet. Fejlrettelser er planlagt i Fase 2."
...
Skriv alt på dansk. Brug GFM-tabeller til teknisk gæld og ydeevne-sektionerne. Brug ### overskrifter til individuelle bugs. BUG-01 SKAL stå som allerførste indgang under en tydelig KRITISK-overskrift med blockquote-advarsel. Hold symptom/årsag-beskrivelserne i plain language uden kodetermer der ikke er forklaret.
  </action>
  <verify>
    <automated>test -f "C:/Users/Jesper/Documents/jq Code/O2A_GUI_Claude/docs/ISSUES.md"</automated>
  </verify>
  <done>docs/ISSUES.md eksisterer med BUG-01 som første indgang under en KRITISK-overskrift med datoen "29. marts 2026" synlig, efterfulgt af BUG-02 til BUG-05 og sektioner for teknisk gæld og ydeevne</done>
</task>

</tasks>

<verification>
- docs/ISSUES.md eksisterer: `test -f docs/ISSUES.md`
- BUG-01 er første indgang og ligger under "KRITISK"-overskrift: Åbn filen og bekræft rækkefølge
- "29. marts 2026" vises eksplicit i BUG-01-indgangen
- Alle fem bugs (BUG-01 til BUG-05) er dokumenteret med symptom, årsag og planlagt løsning
- Teknisk gæld er i en separat sektion som tabel
- Filen er på dansk
</verification>

<success_criteria>
1. Ole kan åbne docs/ISSUES.md og straks se BUG-01 og deadline 29. marts 2026 som det første kritiske punkt
2. Ole kan se alle fem kendte fejl med forklaring der giver ham grundlag for at vurdere alvorlighed
3. Ole kan se hvad der er planlagt rettet i hvilke faser (Fase 2, Fase 3, Fase 4)
4. Teknisk gæld og sikkerhedsnoter er adskilt fra akutte fejl
</success_criteria>

<output>
Efter afslutning, opret `.planning/phases/01-dokumentation/01-03-SUMMARY.md` med en beskrivelse af hvad der blev oprettet og nøglebeslutninger truffet under udførelsen.
</output>
